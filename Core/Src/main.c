...
//    char     text[CHAR_PER_LINE];
  ...
  /* USER CODE BEGIN 2 */
    ...
    display_write_txt2line(HEADING, "Do start", Centered);
    display_update();
    printf("Set Key? " NL);   keyboard_reset(sDev);    
    keyboard_reset(eDev);
    msystem.sync_state = SYNCHRONIZE;
    printf("Set synchronisation state to %s" NL, idxa2str(&synca2str, msystem.sync_state));
  /* USER CODE END 2 */
    static system_state_e read_state = SYNC_RESET;
    read_state = msystem.sync_state;
  /* Infinite loop */
  /* USER CODE BEGIN WHILE */
    while (1) {
    /* USER CODE END WHILE */
        if (read_state != msystem.sync_state){
            printf("System state changed in radio loop to %s "SLOT_PRINT_FMT NL, idxa2str(&synca2str, msystem.sync_state),  msystem.cycle.cycle , ACT_SLOT(&msystem.cycle), ACT_SUB_SLOT(&msystem.cycle));
            read_state = msystem.sync_state;
        }
        if (keyboard_scan((eDev)) >= 0) {
            ...
            if (strlen((char *)rxb.mem) > 0) {
                printf("Received input: \"%s\" "SLOT_PRINT_FMT NL, rxb.mem, msystem.cycle.cycle , ACT_SLOT(&msystem.cycle) ,ACT_SUB_SLOT(&msystem.cycle));
            }
            if (scan) {
                ...
                if (system_state.dirty){
                    state_print(&system_state, "Received state", true);
                }
