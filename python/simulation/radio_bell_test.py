...
def ex_serial(*ports):
    '''
    serial com1 com2 ..
    Test package exchange betweewn com devices.
    '''
    rb = RadioBell(ports[0])
    while (not rb.ready): time.sleep(0.01)
    slot = 1 
    rb.clear_log()
    rb.send_key(b'1', slot)
    rb.send_key(b'1', slot)
    #rb.print_log(slot)
    rb.save_log(slot)
    rb.save_reset(slot)
    af = rb.get_appli_frame(slot)
    print(af)
